...
cudnnConvolutionFwdAlgo_t GetCuDNNFwdAlgo(ConvolutionParameter_CuDNNFwdAlgorithm algo)
{
  switch (algo)
  {
...
cudnnConvolutionBwdDataAlgo_t GetCuDNNBwdDataAlgo(ConvolutionParameter_CuDNNBwdDataAlgorithm algo)
{
  switch (algo)
  {
    ...
    case ConvolutionParameter_CuDNNBwdDataAlgorithm_BWD_DATA_ALGO_2:
      return CUDNN_CONVOLUTION_BWD_DATA_ALGO_2;
    default:
      return CUDNN_CONVOLUTION_BWD_DATA_ALGO_0;
  }
}

cudnnConvolutionBwdFilterAlgo_t GetCuDNNBwdFilterAlgo(ConvolutionParameter_CuDNNBwdFilterAlgorithm algo)
{
  switch (algo)
  {
...
void CuDNNConvolutionLayer<Dtype>::Reshape(
  ...
  for (int i = 0; i < bottom.size(); i++) {
    ...
    if (!this->layer_param_.convolution_param().has_cudnnfwdalgo()) {
      ...
      fwd_algo_[i] = GetCuDNNFwdAlgo(this->layer_param_.convolution_param().cudnnfwdalgo());
    }
    ...
    if (!this->layer_param_.convolution_param().has_cudnnbwdfilteralgo()) {
      ...
      bwd_filter_algo_[i] = GetCuDNNBwdFilterAlgo(this->layer_param_.convolution_param().cudnnbwdfilteralgo());
    }
    //LOG(INFO) << "Backward Filter Convolution using algo: " << (int)bwd_filter_algo_[i];
    ...
    if (!this->layer_param_.convolution_param().has_cudnnbwddataalgo()) {
      ...
      bwd_data_algo_[i] = GetCuDNNBwdDataAlgo(this->layer_param_.convolution_param().cudnnbwddataalgo());
    }
    //LOG(INFO) << "Backward Data Convolution using algo: " << (int)bwd_data_algo_[i];
  ...
  for (int i = 0; i < bottom.size(); i++) {
#if 0
    total_workspace_fwd += workspace_fwd_sizes_[i];
    total_workspace_bwd_data += workspace_bwd_data_sizes_[i];
    total_workspace_bwd_filter += workspace_bwd_filter_sizes_[i];
#else
  ...
  if (max_workspace*(this->group_ * CUDNN_STREAMS_PER_GROUP) > workspaceSizeInBytes) {

    LOG(INFO) << "Reallocating workspace storage: " << (max_workspace*(this->group_ * CUDNN_STREAMS_PER_GROUP));
    workspaceSizeInBytes = max_workspace*(this->group_ * CUDNN_STREAMS_PER_GROUP);
    ...
    for (int g = 0; g < (this->group_ * CUDNN_STREAMS_PER_GROUP); g++) {
      workspace[g] = (char *)workspaceData + g*max_workspace; 
    }
  }
