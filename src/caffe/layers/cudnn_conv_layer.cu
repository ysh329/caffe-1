...
#ifdef USE_CNMEM
      Caffe::mallocGPU(&workspace[0], workspace_fwd_sizes_[i], stream_[0]);
#endif
      // Filters.
...
#ifdef USE_CNMEM
      Caffe::freeGPU(workspace[0], stream_[0]);
#endif
      // Bias.
      if (this->bias_term_) {
...
void CuDNNConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    for (int g = 0; g < this->group_; g++) {
      ...
      if (this->param_propagate_down_[0]) {
#ifdef USE_CNMEM
        Caffe::mallocGPU(&workspace[0], workspace_bwd_filter_sizes_[i],
                         stream_[0]);
#endif
        const Dtype* bottom_data = bottom[i]->gpu_data();
...
#ifdef USE_CNMEM
        Caffe::mallocGPU(&workspace[0], workspace_bwd_data_sizes_[i],
                         stream_[0]);
#endif
